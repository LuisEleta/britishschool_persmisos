angular.module('angular', []).controller("comedorcontroller", function ($scope, $http) {
    ...
    $scope.btn_crear_permiso_new = function () {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $vacio = 0;
                if (name_permiso.value == '') {
                    $vacio++;
                    $("#name_permiso").addClass("vacio");
                } else {
                    $("#name_permiso").removeClass("vacio");
                }
                if ($vacio > 0) {
                    Swal.fire({
                        title: 'Error',
    ...
    $scope.btn_crear_motivo_new = function () {
        $vacio = 0;
        $time = '';
        if (name_motivo.value == '') {
            $vacio++;
            $("#name_motivo").addClass("vacio");
        } else {
            $("#name_motivo").removeClass("vacio");
        }
        if (tipo_tiempo.value == 'DI' && tiempo_dias.value == '') {
            $vacio++;
            $("#tiempo_dias").addClass("vacio");
        } else {
            $("#tiempo_dias").removeClass("vacio");
            $time = tiempo_dias.value;
        }
        if (tipo_tiempo.value == 'HR' && tiempo_hr.value == '') {
            $vacio++;
            $("#tiempo_hr").addClass("vacio");
        } else {
            $("#tiempo_hr").removeClass("vacio");
            $time = tiempo_hr.value;
        }
    ...
    $scope.btn_editar_motivo_new = function () {
        $vacio = 0;
        $time = '';
        if (name_motivo_e.value == '') {
            $vacio++;
            $("#name_motivo_e").addClass("vacio");
        } else {
            $("#name_motivo_e").removeClass("vacio");
        }
        if (tipo_tiempo_e.value == 'DI' && tiempo_dias_e.value == '') {
            $vacio++;
            $("#tiempo_dias_e").addClass("vacio");
        } else {
            $("#tiempo_dias_e").removeClass("vacio");
            $time = tiempo_dias_e.value;
        }
        if (tipo_tiempo_e.value == 'HR' && tiempo_hr_e.value == '') {
            $vacio++;
            $("#tiempo_hr_e").addClass("vacio");
        } else {
            $("#tiempo_hr_e").removeClass("vacio");
            $time = tiempo_hr_e.value;
        }
    ...
    $scope.btn_agg_supervisor = function () {
        $vacio = 0;
        if (rut_supervisor.value == '') {
            $vacio++;
            $("#rut_supervisor").addClass("vacio");
        } else {
            $("#rut_supervisor").removeClass("vacio");
        }
        if (name_supervisor.value == '') {
            $vacio++;
            $("#name_supervisor").addClass("vacio");
        } else {
            $("#name_supervisor").removeClass("vacio");
        }
        if ($vacio > 0) {
            Swal.fire({
                title: 'Error',
    ...
    $('#tipo_tiempo_e').on('change', (function () {
    ...
    $scope.agg_vinculo_neew = function ($id, $name) {
        ...
        console.log("php/supervisores/curd_supervisores.php?opc=trear_vinculados&id=" + $id);
    ...
    $scope.btn_agg_viculacion = function () {
        $vacio = 0;
        if (name_sup_reonly.value == '') {
            $vacio++;
            $("#name_sup_reonly").addClass("vacio");
        } else {
            $("#name_sup_reonly").removeClass("vacio");
        }
        if (rut_funcionario.value == '') {
            $vacio++;
            $("#rut_funcionario").addClass("vacio");
        } else {
            $("#rut_funcionario").removeClass("vacio");
        }
        if (name_funcionaro.value == '') {
            $vacio++;
            $("#name_funcionaro").addClass("vacio");
        } else {
            $("#name_funcionaro").removeClass("vacio");
        }
        if ($vacio > 0) {
            Swal.fire({
                title: 'Error',
    ...
    $scope.btn_editar_funcionario = function () {
        $vacio = 0;
        if (nombre_funcionario.value == '') {
            $vacio++;
            $("#nombre_funcionario").addClass("vacio");
        } else {
            $("#nombre_funcionario").removeClass("vacio");
        }
        if (rut_funcionario_ed.value == '') {
            $vacio++;
            $("#rut_funcionario_ed").addClass("vacio");
        } else {
            $("#rut_funcionario_ed").removeClass("vacio");
        }
        if (correo_funcionario.value == '') {
            $vacio++;
            $("#correo_funcionario").addClass("vacio");
        } else {
            $("#correo_funcionario").removeClass("vacio");
        }
        $con = 0;
        ...
        if ($vacio > 0) {
            ...
            $http.get("php/funcionarios/crud_funcionarios.php?opc=editar_funcionario&id=" + $id_duncionario + "&correo=" + correo_funcionario.value
                ...
                function (response) {
                    $http.get("php/parametros_permisos/actualizar_fun_unico.php?id=" + $id_duncionario).success(
                        function (response) {
                            $http.get("php/funcionarios/crud_funcionarios.php?opc=todos_funcionarios").success(
                                function (response) {
                                    if (response[0].id) {
    ...
    $("#buscador_f").keyup(function () {
        for (let i in $scope.list_funcionarios) {
            if (buscador_f.value !== "") {
                ...
                if (index < 0) {
                    index = $scope.list_funcionarios[i].nombre.toUpperCase().indexOf(buscador_f.value.toUpperCase());
                }
                if (index < 0) {
                    index = $scope.list_funcionarios[i].apellido_p.toUpperCase().indexOf(buscador_f.value.toUpperCase());
                }
                if (index < 0) {
                    index = $scope.list_funcionarios[i].correo.toUpperCase().indexOf(buscador_f.value.toUpperCase());
                }
                if (index < 0) {
                    index = $scope.list_funcionarios[i].tipo_f.toUpperCase().indexOf(buscador_f.value.toUpperCase());
                }
                if (index < 0) {
                    index = $scope.list_funcionarios[i].horario.toUpperCase().indexOf(buscador_f.value.toUpperCase());
                }
                if (index >= 0) {
    ...
    $scope.btn_listado_permisosnew = function () {
        $(".caja-dere").addClass("oculto");
        $("#div_list_permisosnew").removeClass("oculto");
        $http.get("php/permisos/crud_permisos_funcionarios.php?opc=traer_todos_los_permisos").success(
            function (response) {
                if (response[0].id) {
                    $scope.list_permisos = response;
    ...
    $("#buscador_lp").keyup(function () {
        for (let i in $scope.list_permisos) {
            if (buscador_lp.value !== "") {
                ...
                if (index < 0) {
                    index = $scope.list_permisos[i].name.toUpperCase().indexOf(buscador_lp.value.toUpperCase());
                }
                if (index < 0) {
                    index = $scope.list_permisos[i].tipo_permiso.toUpperCase().indexOf(buscador_lp.value.toUpperCase());
                }
                if (index < 0) {
                    index = $scope.list_permisos[i].motivo.toUpperCase().indexOf(buscador_lp.value.toUpperCase());
                }
                if (index < 0) {
                    index = $scope.list_permisos[i].status.toUpperCase().indexOf(buscador_lp.value.toUpperCase());
                }
                if (index < 0) {
                    index = $scope.list_permisos[i].fi.indexOf(buscador_lp.value.toUpperCase());
                }
                if (index < 0) {
                    index = $scope.list_permisos[i].ff.indexOf(buscador_lp.value.toUpperCase());
                }
                if (index >= 0) {
    ...
    $("#buscador_lp2").keyup(function () {
        for (let i in $scope.list_permisos) {
            if (buscador_lp2.value !== "") {
                index = 0;
                index = $scope.list_permisos[i].rut.indexOf(buscador_lp2.value);
                if (index < 0) {
                    index = $scope.list_permisos[i].name.toUpperCase().indexOf(buscador_lp2.value.toUpperCase());
                }
                if (index < 0) {
                    index = $scope.list_permisos[i].tipo_permiso.toUpperCase().indexOf(buscador_lp2.value.toUpperCase());
                }
                if (index < 0) {
                    index = $scope.list_permisos[i].motivo.toUpperCase().indexOf(buscador_lp2.value.toUpperCase());
                }
                if (index < 0) {
                    index = $scope.list_permisos[i].status.toUpperCase().indexOf(buscador_lp2.value.toUpperCase());
                }
                if (index < 0) {
                    index = $scope.list_permisos[i].fi.indexOf(buscador_lp2.value.toUpperCase());
                }
                if (index < 0) {
                    index = $scope.list_permisos[i].ff.indexOf(buscador_lp2.value.toUpperCase());
                }
                if (index >= 0) {
                    $scope.list_permisos[i].foto = i;
                } else {
    ...
    $('#hr_semanal').on('change', (function () {
        //console.log('cambia');
        if (hr_semanal.value > 37) {
            hr_anno.value = 27;
        } else {
            hr_anno.value = Math.ceil((hr_semanal.value / 38) * 27);
        }
    }));
...
//------------------------------------------------------------------------------------------------------------------
    ...
    $('#cbox1').on('change', (function () {
        if ($('#cbox1').prop('checked')) {
            ...
            if (hi.value != '' && hf.value != '') {
                ...
                if (parseInt($min_total) < 10) {
                    $min_total = '0' + $min_total;
                }
                if (parseInt($hr_total) < 10) {
                    $hr_total = '0' + $hr_total;
                }
    ...
    $('#fi').on('change', (function () {
        ...
        if ($tipo_tiempo == 'DI') {
            ...
            if (($m + '').length < 2) {
                $m = '0' + $m;
                console.log('agregue mes')
            }
            if (($d + '').length < 2) {
                $d = '0' + $d;
                console.log('agregue dia')
            }
            ff.value = $a + '-' + $m + '-' + $d;
        }
        $scope.$apply();
    ...
    $('#hf').on('change', (function () {
            ...
            if (hi.value != '' && hf.value != '' && fi.value == ff.value) {
                if ($scope.horario[0].nombre == 'Normal') {
                    ...
                    if (parseInt($hr_ingresada2) < parseInt($hora_inicio_receso) && parseInt($hr_ingresada) > parseInt($hora_fin_receso) && fi.value == ff.value) {
                        $ban = 1;
                    }
                }
                ...
                if ($ban == 1) {
                    ...
                    $min_receso = $scope.horario[0].receso_t.charAt(3) + $scope.horario[0].receso_t.charAt(4);
                    $minfin = $minfin - $min_receso;
                    if (parseInt($minfin) < 0) {
                        $minfin = parseInt($minfin) + parseInt(60);
                        $hrfin = parseInt($hrfin) - parseInt(1);
                    }
                    $hrfin = $hrfin - $hr_receso;
                }
                $min_total = parseInt($minfin) - parseInt($minini);
                $hr_total = parseInt($hrfin) - parseInt($hrini);
                if (parseInt($min_total) < 0) {
                    $min_total = parseInt(60) + parseInt($min_total);
                    $hr_total = $hr_total - 1;
                }
                if (parseInt($min_total) < 10) {
                    $min_total = '0' + $min_total;
            ...
            if (hi.value != '' && hf.value != '' && fi.value != ff.value) {
                ...
                if (parseInt($minutos_acumulados) < 10) {
                    $minutos_acumulados = '0' + $minutos_acumulados;
                }
                if (parseInt($horas_acumuladas) < 10) {
                    $horas_acumuladas = '0' + $horas_acumuladas;
                }
    ...
    $scope.btn_crear_permiso2 = function () {
        $vacio = 0;
        if (tipo_permiso.value == '0') {
            $vacio++;
            $("#tipo_permiso").addClass("vacio");
        } else {
            $("#tipo_permiso").removeClass("vacio");
        }
        if (motivo.value == '0') {
            $vacio++;
            $("#motivo").addClass("vacio");
        } else {
            $("#motivo").removeClass("vacio");
        }
        if (fi.value == '0') {
            $vacio++;
            $("#fi").addClass("vacio");
        } else {
            $("#fi").removeClass("vacio");
        }
        if ($vacio > 0) {
            Swal.fire({
                title: 'Error',
    ...
    $scope.div_calendario = function () {
        if (fi_calendario.value == '') {
            $("#fi_calendario").addClass("vacio");
            console.log('ff1');
            return 0;
        } else {
            $("#fi_calendario").removeClass("vacio");
        }
        if (ff_calendario.value == '') {
            $("#ff_calendario").addClass("vacio");
            console.log('ff');
            return 0;
        } else {
            $("#ff_calendario").removeClass("vacio");
        }
        ...
        $scope.list_calendario = [];
        if (parseInt($fecha_i_c) > parseInt($fecha_f_c)) {
    ...
    $scope.modal_dia = function () {
        ...
        $http.get("php/permisos/crud_permisos_funcionarios.php?opc=permisos_calendario&fi=" + $fecha_i_c + "&ff=" + $fecha_f_c + "&tipo=" + tipo_funcionario_c.value).success(
            function (response) {
                if (response[0].id) {
                    $scope.list_calendario = response;
                    for (let i in $scope.list_calendario) {
                        if ($scope.list_calendario[i].tipo_f == 'Auxiliar' && (document.getElementById('cbox_auxiliar').checked) == false) {
                            $scope.list_calendario[i].tipo_f = 'x oculto';
                        }
                        if ($scope.list_calendario[i].tipo_f == 'Auxiliar' && (document.getElementById('cbox_auxiliar').checked) == true) {
                            $scope.list_calendario[i].tipo_f = 'A';
                        }
                        if ($scope.list_calendario[i].tipo_f == 'Docente' && (document.getElementById('cbox_docente').checked) == false) {
                            $scope.list_calendario[i].tipo_f = 'x oculto';
                        }
                        if ($scope.list_calendario[i].tipo_f == 'Docente' && (document.getElementById('cbox_docente').checked) == true) {
                            $scope.list_calendario[i].tipo_f = 'D';
                        }
                        if ($scope.list_calendario[i].tipo_f == 'No Docente' && (document.getElementById('cbox_no_docente').checked) == false) {
                            $scope.list_calendario[i].tipo_f = 'x oculto';
                        }
                        if ($scope.list_calendario[i].tipo_f == 'No Docente' && (document.getElementById('cbox_no_docente').checked) == true) {
                            $scope.list_calendario[i].tipo_f = 'ND';
                        }
    ...
    $scope.modal_pre_calendario = function () {
        ...
        Swal.fire({
            ...
            title: 'Espere',
            html: 'Estamos cargando su calendario.',
            timer: 27000,
...
//--------------------------------------------------------------------------------------------------------------------------------
    ...
    fecha_calendario.value = currentYear + '-' + $mm + '-01';
    ...
    const writeMonth = (month) => {
        ...
        for (let i = 1; i <= getTotalDays(month); i++) {
            ...
            let day_n = i;
            let mes_n = month + 1;
            if (parseInt(day_n) < 10) {
                day_n = '0' + day_n.toString();
            }
            if (parseInt(mes_n) < 10) {
                mes_n = '0' + mes_n.toString();
            }
            ...
            $http.get("php/permisos/crud_permisos_funcionarios.php?opc=permisos_calendario_dia&fecha=" + $fech + "&dia=" + day_n).success(
                function (response) {
                    $dias_mes[i] = ` <div onclick="myFunction('${response[0].fecha}')" class="calendar__date calendar__item"><spam class="n_dia">${response[0].dia}
                                            <div class="btn-azul check_docente m-2 p-2 ${$v_d}">
                                              <label class="label_input">${response[0].docentes} Docentes</label>
                                            </div>
                                            <div class="btn-rojizo cbox_no_docente m-2 p-2 ${$vnd}">
                                              <label class="label_input">${response[0].nodocentes} No Docentes</label>
                                            </div>
                                            <div class="btn-verde cbox_auxiliar m-2 p-2 ${$v_a}">
        ...
        setTimeout(function () {
            for (let j = 1; j <= getTotalDays(month); j++) {
                dates.innerHTML += $dias_mes[j];
            }
        }, 27000);
    ...
    const lastMonth = () => {
        ...
        $("#cbox_docente").prop("checked", true);
        $("#cbox_no_docente").prop("checked", true);
        $("#cbox_auxiliar").prop("checked", true);
    ...
    $('#fecha_calendario').on('change', (function () {
        monthNumber = (fecha_calendario.value.charAt(5) + fecha_calendario.value.charAt(6)) - 1;
        currentYear = fecha_calendario.value.charAt(0) + fecha_calendario.value.charAt(1) + fecha_calendario.value.charAt(2) + fecha_calendario.value.charAt(3)
        setNewDate();
    }));
...
//--------------------------------------------------------------------------------------------------------------------------
    ...
    $('#cbox_docente').on('change', (function () {
        if (document.getElementById('cbox_docente').checked) {
            $(".check_docente").removeClass("oculto");
            $v_d = '';
        } else {
            $(".check_docente").addClass("oculto");
            $v_d = 'oculto';
        }
    }));

    $('#cbox_no_docente').on('change', (function () {
        if (document.getElementById('cbox_no_docente').checked) {
            $(".cbox_no_docente").removeClass("oculto");
            $vnd = '';
        } else {
            $vnd = 'oculto';
            $(".cbox_no_docente").addClass("oculto");
        }
    }));

    $('#cbox_auxiliar').on('change', (function () {
        if (document.getElementById('cbox_auxiliar').checked) {
            $(".cbox_auxiliar").removeClass("oculto");
            $v_a = '';
        } else {
            $v_a = 'oculto';
            $(".cbox_auxiliar").addClass("oculto");
        }
    }));

    $("#info_f").keyup(function () {
        for (let i in $scope.list_funcionarios) {
            if (info_f.value !== "") {
                index = 0;
                index = $scope.list_funcionarios[i].rut.indexOf(info_f.value);
                if (index < 0) {
                    index = $scope.list_funcionarios[i].nombre.toUpperCase().indexOf(info_f.value.toUpperCase());
                }
                if (index < 0) {
                    index = $scope.list_funcionarios[i].apellido_p.toUpperCase().indexOf(info_f.value.toUpperCase());
                }
                if (index < 0) {
                    index = $scope.list_funcionarios[i].supe.toUpperCase().indexOf(info_f.value.toUpperCase());
                }
                if (index < 0) {
                    index = $scope.list_funcionarios[i].tipo_f.toUpperCase().indexOf(info_f.value.toUpperCase());
                }
                if (index >= 0) {
                    $scope.list_funcionarios[i].foto = i;
                } else {
    ...
    $scope.btn_eliminar_permiso = function ($id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $scope.list_permisos = [];
                $http.get("php/permisos/crud_permisos_funcionarios.php?opc=eliminar_permiso_funcionario_admin&id=" + $id).success(
                    function (response) {
                        $http.get("php/parametros_permisos/actualizar_horas.php").success(
                            function (response) {
                                $http.get("php/permisos/crud_permisos_funcionarios.php?opc=traer_todos_los_permisos").success(
